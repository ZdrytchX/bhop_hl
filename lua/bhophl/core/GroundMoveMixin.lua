...
local kMaxAirAccel = 0--0.54

local kStopFriction = 6
local kStopSpeed = 3.125--4

local kMaxAirVeer = 1.3

local kFixedTimeStep = 0.005 -- 200 fps
-- min ~13 FPS assumed, otherwise players will move slower
Print("Using Nin's FixedMovementStep: 200 FPS movement")
...
function GroundMoveMixin:__initmixin()
    ...
    self.timeLastUpdate = 0--undefined elsewise
end
...
local function ForwardControl(self, deltaTime, velocity)
    ...
    local airControl = self:GetAirControl()
    ...
    if airControl > 0 then
        ...
        velocity:Add(wishDir * deltaTime * airControl * dot * dot)
        velocity.y = 0
        velocity:Normalize()
...
local function Accelerate(self, input, velocity, deltaTime)
    ...
    local currentSpeed = velocity:DotProduct(wishDir)--math.min(velocity:GetLength(), velocity:DotProduct(wishDir))
...
--FIXME Apparently when jumping a class without air control gets a boost
    ...
    if addSpeed > 0 then
        ...
        local accel = self.onGround and groundFraction * self:GetAcceleration() or 0 --self:GetAirControl()
...
--    end
    ...
    if not self.onGround then
       ...
       if not self.GetHasFallAccel or self:GetHasFallAccel() then
            ...
            wishDir.y = 0
            local fallAccel = 0--math.max(-velocity.y, 0) * deltaTime * kFallAccel
    ...
    if not self.onGround then
        ...
        local speedScalar = 0--1 - Clamp(velocity:GetLengthXZ() / maxSpeedTable.maxSpeed, 0, 1) ^ 2
...
function GroundMoveMixin:GetFriction(input, velocity)
    ...
    if self.onGround --[[and velocity:GetLength() < kStopSpeed]] then
        frictionScalar = math.max(kStopFriction * self:GetAcceleration() / 10, frictionScalar)-- self:GetGroundFriction()
    end
...
--Patched from Nin's FixedMovementStep mod
    --local deltaTime = input.time -- math.min(kMaxDeltaTime, input.time)
    local deltaTime = kFixedTimeStep--Smoother input
    self.timeLastUpdate = self.timeLastUpdate + input.time
    while self.timeLastUpdate > 0 do
      if self.timeLastUpdate > deltaTime then
        self.timeLastUpdate = self.timeLastUpdate - deltaTime
      else
        deltaTime = self.timeLastUpdate
